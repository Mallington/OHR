...
/**
 * This class is used for keeping track of all of the pixels in a formation, it
 * can be used to mark the bound for characters
 *
 * @author mathew
 */
public class PixelFormation {
    ...
    public Rectangle getBounds() { // need to implement
        ...
        if (modified) {
            ...
            for (Point p : POINTS) {
                if (p.x < topLeft.x) {
                    topLeft = new Point(p.x, topLeft.y);
                }
                if (p.y < topLeft.y) {
                    topLeft = new Point(topLeft.x, p.y);
                }

                if (p.x > bottomRight.x) {
                    bottomRight = new Point(p.x, bottomRight.y);
                }
                if (p.y > bottomRight.y) {
                    bottomRight = new Point(bottomRight.x, p.y);
                }
            }
