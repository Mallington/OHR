...
/**
 * The purpose of this class is to sort a given set of pixel formations so that
 * they are in the order that they would be read on a page, so that they are
 * outputted in the right order when the raw text is outputted by the
 * FormRecogntion class
 *
 * @author mathew
 */
public class LineSorter {
    ...
    private List<PixelFormation> sortByX(List<PixelFormation> toSort) {
        ...
        while (toSort.size() > 0) {
            ...
            for (int i = 1; i < toSort.size(); i++) {
                if (toSort.get(i).getBounds().getX() < toSort.get(biggest).getBounds().getX()) {
                    biggest = i;
                }
            }
    ...
    private List<PixelFormation> sortByY(List<PixelFormation> toSort) {
        ...
        while (toSort.size() > 0) {
            int biggest = 0;
            for (int i = 1; i < toSort.size(); i++) {
                if (toSort.get(i).getBounds().getY() < toSort.get(biggest).getBounds().getY()) {
                    biggest = i;
                }
            }
    ...
    /**
     * The purpose of this methods is to take the detected lines and put all of
     * the character formations from the list in order while recording the start
     * and end of each line by storing '\n' as an additional character to be
     * inserted at a particular index.
     *
     * @param pf
     * @param lines
     * @return
    ...
    public static SortingOutput sortLines(List<PixelFormation> pf, List<Line> lines) {
        ...
        while (lines.size() > 0) {
            ...
            for (int i = 1; i < lines.size(); i++) {
                if (pf.get(lines.get(i).start).getBounds().getY() < pf.get(lines.get(smallest).start).getBounds().getY()) {
                    smallest = i;
                }
            }
    ...
    /**
     * This methods first sorts the chars by the y-axis, then the x-axis so that
     * the characters are accessed horizontally and then the most vertical pixel
     * formations are accessed first, so once the top left formations is
     * identified the program runs through each formation where there bounds
     * intersect within the same Y-axis range, proving the formations are on the
     * same line, When the program gets to the end of the line it records the
     * start and end index in the Line class so that it can be used later for
     * sorting.
     *
     * @return
     */
