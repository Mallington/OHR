...
/**
 * The function of this class is to detect the character locations, binarise
 * each one, run it through the neural network to evaluate what each char is,
 * work out how many lines of text there is on the page and output the detected
 * chars using the Recognition output class.
 *
 * @author mathew
 */
public abstract class FormRecognition {
    ...
    /**
     * The class containing the most up to date outputs such as char locations
     * and order for real-time viewing
     */
    ...
    /**
     * Abstract method to be executed upon an error with the recognition
     * parameters
     */
    public abstract void error();

    /**
     * This abstract method is specified by the instigator class so that when
     * progress is updated, this method executes specialised code tailored to
     * the context the class was instigated in, for example this could be used
     * to update a GUI or simply output into a console.
     *
    ...
    /**
     * When executed, the processing starts on a separate thread so that the
     * instigating thread does not freeze up, to create a real-time update on
     * processing this methods uses the abstract classes specified by the
     * instigating class so that progress is constantly updated
     *
     * @return
     */
    ...
    /**
     * This methods crops the image according to the pixel bound, converts it to
     * a 30x30 binary image and inputs it into the neural netork
     *
     * @param img image to be cropped
     * @param p Pixel bound used for cropping
     * @param neural Neural network weightings used for evaluating each
     * character
