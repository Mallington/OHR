...
/**
 * In this particular extension of image view the Crop Panel draws a crop region
 * over the image view which the user can move and position to crop an image, it
 * also provides a binary view of the image and it is also able to output auto
 * detected character locations
 *
 * @author 160471
 */
public class CropPanel extends ImageView {
    ...
    /**
     * Initialises its parent class with the canvas and the image to be drawn
     * then calculates the crop bound location so that the parent can render the
     * graphics
     *
    ...
    /**
     * This is used to calculate the exact size and location of the crop bound
     * to fit a recognised character
     *
    ...
    /**
     * Modifies the image based on a set of parameters to provide the user with
     * an image if the view
     *
     * @param threshold The point at which it turns a particular pixel black or
     * white
     * @param binarise Specifies whether to binarise the image or not
     * @param detectForms Specifies whether to detect the character locations
     * and display them
     */
    ...
    /**
     * Overrides the abstract methods and draws the pixel formations and the the
     * crop window over the existing image view.
     *
    ...
    private void drawFormationBounds(GraphicsContext g, List<PixelFormation> formation) {
        if (this.PRIMARY_FORM == null) {
            drawFormationBounds(g, formation, X_OFF, Y_OFF, SCALE, null);
        }
    ...
    void pressed(MouseEvent event) {
        ...
        if (intersectsDoc && !intersectsCrop) {
            moveCrop = false;
        }
    }
