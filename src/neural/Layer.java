...
/**
 * This class contains all of the neurons that are each independently
 * responsible for a particular detection/ classification, this used as the
 * containing class, so that this entire object can be serialised and written to
 * a file
 *
 * @author mathew
 */
public class Layer implements Serializable {
    ...
    /**
     * This adjusts the weightings of each neuron in the network, so that the
     * produced output in each will be closer to a desired value
     *
     * @param train
     */
    public void backwardProp(TrainingSet train) {
        ...
        for (int i = 0; i < currentOut.size(); i++) {
            System.out.println(NEURONS.get(i).toString() + " | Out: " + currentOut.get(i));
        }
        ...
        for (int i = 0; i < currentOut.size(); i++) {
            deltas.add(train.EXPECTED_OUTPUT.get(i) - currentOut.get(i));
        }
